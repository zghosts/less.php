...
class ParserTest
{
    ...
    public function testLessJsCssGeneration($dir, $less, $css, $map)
    {
        ...
        echo '<a class="filename" href="?dir=' . rawurlencode($this->dir) . '&amp;file=' . rawurlencode(
                $basename
            ) . '">File: ' . $basename . '</a>';
    ...
    public function ObjBuffer()
    {
        ...
        if (!empty($obj_buffer)) {
            ...
            echo '<textarea id="object_comparison">' . htmlspecialchars(
                    $obj_buffer,
                    ENT_COMPAT,
                    'UTF-8',
                    false
    ...
    public function Summary()
    {
        ...
        echo '<fieldset><legend>Memory</legend>' . self::FormatBytes($memory) . ' (' . number_format(
                $memory
            ) . ')</fieldset>';

        //max memory usage
        $memory = memory_get_peak_usage();
        echo '<fieldset><legend>Memory Peak</legend>' . self::FormatBytes($memory) . ' (' . number_format(
                $memory
            ) . ')</fieldset>';

        //time
        echo '<fieldset><legend>Time (PHP):</legend>' . self::microtime_diff(
                phpless_start_time,
                microtime()
            ) . '</fieldset>';
        echo '<fieldset><legend>Time (Request)</legend>' . self::microtime_diff(
