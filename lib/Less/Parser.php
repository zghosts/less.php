...
class Less_Parser
{
    ...
    private function parseEntitiesLiteral()
    {
        return $this->MatchFuncs(
            array('parseEntitiesDimension', 'parseEntitiesColor', 'parseEntitiesQuoted', 'parseUnicodeDescriptor')
        );
    }
    ...
    private function parseAttribute()
    {
        ...
        if ($op) {
            $val = $this->match(
                array('parseEntitiesQuoted', '/\\G[0-9]+%/', '/\\G[\w-]+/', 'parseEntitiesVariableCurly')
            );
        }
    ...
    private function parseNameValue()
    {
        ...
        $match = $this->MatchReg(
            '/\\G([a-zA-Z\-]+)\s*:\s*([\'"]?[#a-zA-Z0-9\-%\.,]+?[\'"]?) *(! *important)?\s*([;}])/'
        );
        if ($match) {
    ...
    private function parseCondition()
    {
        ...
        if ($a) {
            ...
            $this->expectChar(')');
            return $this->MatchReg('/\\Gand/') ? $this->NewObj3(
                'Less_Tree_Condition',
                array('and', $c, $this->parseCondition())
            ) : $c;
        }
    }
