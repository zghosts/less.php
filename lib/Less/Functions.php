...
class Less_Functions
{
    ...
    private function _minmax($isMin, $args)
    {
        ...
        return new Less_Tree_Anonymous(($isMin ? 'min(' : 'max(') . implode(
    ...
    {
        return ($n instanceof Less_Tree_Dimension) && $n->unit->is(
            (property_exists($unit, 'value') ? $unit->value : $unit)
        ) ? new Less_Tree_Keyword('true') : new Less_Tree_Keyword('false');
    }
